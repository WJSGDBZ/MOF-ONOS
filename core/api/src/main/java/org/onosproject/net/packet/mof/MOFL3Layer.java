 ...
 public class MOFL3Layer extends BasePacket {
    ...
    */
    ...
    public static MOFL3Layer parse(int type, byte[] data, int offset){
        ...
        int start = data.length - offset;
        switch (type) {
            case 0x0800:
                l3.setIpv4_E_Protocol(Ipv4_E_Protocol.read(Unpooled.wrappedBuffer(data, offset, start)));
                break;
            case 0x86DD:
                l3.setIpv6_E_Protocol(Ipv6_E_Protocol.read(Unpooled.wrappedBuffer(data, offset, start)));
                break;
            default:
